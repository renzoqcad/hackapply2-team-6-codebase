...
async function extractTextFromImage(file: File): Promise<string> {
  try {
    ...
    const prompt = 'Extract all text from this image. Return only the raw text content, preserving line breaks and structure. Do not add any explanations or formatting.';
...
async function extractTextFromPDF(file: File): Promise<string> {
  try {
    ...
    const prompt = 'Extract all text content from this PDF document. Return only the raw text content, preserving structure and line breaks. Do not add any explanations.';
...
export function extractMiroBoardId(miroUrl: string): string | null {
  try {
    ...
    const match = miroUrl.match(/miro\.com\/app\/board\/([a-zA-Z0-9]+)/);
