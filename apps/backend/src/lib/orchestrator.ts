...
export async function processInput(
  ...
  try {
    ...
    if (input.type === 'file') {
      ...
      console.log(`[Orchestrator] Extracted ${result.metadata?.wordCount || 0} words from ${result.type} file`);
    } else {
      // Miro URL
      ...
      console.log(`[Orchestrator] Extracted content from Miro board: ${boardId}`);
    }
